const txt = `
البريد الإلكتروني
التجارة الإلكترونية
متجر
عربة التسوق
الدفع
بطاقات
خرائط
خرائط جوجل
التقويم
ملاحظات
تودوس
جهات الاتصال
خلط
الرسوم البيانية
عربات غوغل
Recharts
رد فيس
الرد-مخطط-2
الرد-تريند
Echart
إستمارات
إدخال
محرر
نماذج مع التحقق من الصحة
تقدم
زر
التبويب
مربع
Radiobox
تحويل
الإكمال التلقائي
خيارات مربع
عناصر واجهة المستخدم
شارة
بطاقة
إحتفال صاخب مخمور
انهدام
البوب
تلميح
بطاقة
الجدول الزمني
اسقاط
ترقيم الصفحات
تقييم
شجرة
العناصر المتقدمة
تواريخ الاستجابة
كود مرآة
وبي رافع
إسقاط المنطقة
ردود الفعل
محزر
شكلي
رسالة
إعلام
تأكيد البوب
غزل
الجداول
جداول النمل
صفحات
500
404
تسجيل الدخول
سجل
نسيت كلمات المرور
إعادة تعيين كلمات المرور
فاتورة
مستويات القائمة
البند 1
البند 2
الخيار 1
الخيار 2
الخيار 3
الخيار 4
صفحة فارغة
غير اللغة
موضوع الجلاد
الشريط الجانبي
Topbar
خلفية
العنوان الأساسي
نص النجاح
نص المعلومات
نص تحذير
نص خطأ
نوع التنبيه القابل للإغلاق
هذا إشعار تحذير حول كتابة النصوص.
هذه رسالة خطأ حول كتابة النصوص.
رمز نوع التنبيه
رمز معلومات نوع التنبيه
نصائح النجاح
وصف مفصل ونصائح حول الكتابة الناجحة.
ملاحظات إعلامية
وصف إضافي ومعلومات حول كتابة النصوص.
تحذير
خطأ
مشروط واحد مع تخصيص تذييل الصفحة
مربع حوار أساسي مشروط.
نجاح
معلومات
خطأ
تحذير
شكلي
حوار مشروط التأكيد
حوار مشروط بسيط
رسالة
رسالة عادية
الرسائل العادية كتعليقات.
عرض الرسالة العادية
أنواع أخرى من الرسالة
رسائل النجاح وأنواع الأخطاء والإنذارات.
تخصيص المدة
أوستميز مدة عرض الرسالة من الافتراضي 1.5s إلى 10s.
تخصيص مدة العرض
رسالة التحميل
عرض مؤشر تحميل عالمي، يتم رفضه في حد ذاته بشكل غير متزامن.
عرض مؤشر التحميل
إعلام
الأساسية
أبسط الاستخدام الذي إغلاق مربع الإعلام بعد 4.5S.
افتح مربع الإشعارات
المدة التي يتم بعدها إغلاق مربع الإشعارات
يمكن استخدام المدة لتحديد المدة التي يبقى فيها الإشعار مفتوحا. بعد انقضاء المدة الزمنية، يتم إغلاق الإشعار تلقائيا. إذا لم يتم تحديد القيمة الافتراضية هي 4.5 ثانية. إذا قمت بتعيين القيمة إلى 0، لن يتم إغلاق مربع الإشعار تلقائيا تلقائيا.
إعلام مع رمز
مربع إعلام مع رمز على الجانب الأيسر.
إعلام مع رمز مخصص
الرسائل العادية كتعليقات.
إعلام مع زر مخصص
الرسائل العادية كتعليقات.
تأكيد البوب
تأكيد أساسي
المثال الأساسي.
حذف
إعلام مع رمز مخصص
الرسائل العادية كتعليقات.
TL
أعلى
TR
LT
اليسار
رطل
RT
حق
RB
BL
الأسفل
BR
غزل
حجم تدور
تدور مع الخلفية
تدور مع وصف الخلفية
تحميل الدولة:
عنوان رسالة التنبيه
مزيد من التفاصيل حول سياق هذا التنبيه.
إدخال
الاستخدام الأساسي
مثال الاستخدام الأساسي.
ثلاثة أحجام من الإدخال
هناك ثلاثة أحجام من مربع الإدخال: كبير (42px)، الافتراضي (35px) والصغيرة (30px). ملاحظة: داخل النماذج، يتم استخدام الحجم الكبير فقط.
مجموعة الإدخال
الإدخال. مثال المجموعة ملاحظة: أنت لا تحتاج كول للتحكم في العرض في الوضع المضغوط.
تمرير الارتفاع ليتناسب مع المحتوى
أوتوسيزي دعامة لنوع تكستاريا من المدخلات يجعل ارتفاع لضبط تلقائيا استنادا إلى المحتوى. يمكن توفير كائن خيارات ل أوتوسيزي لتحديد الحد الأدنى والحد الأقصى لعدد الخطوط التي سوف تكستاريا ضبط تلقائيا.
علامة التبويب ما قبل / المشاركة
استخدام بري & أمب؛ نموذج علامات تبويب المشاركات ..
ناحية النص
بالنسبة لحالات إدخال المستخدم متعددة الخطوط، يمكن استخدام المدخلات التي لها قيمة دعامة قيمة تكستاريا.
بحث
مثال لإنشاء مربع بحث عن طريق تجميع إدخال قياسي باستخدام زر بحث
محرر
نموذج التحقق المخصص
فشل
يجب أن يكون مزيج من الأرقام & أمب؛ الحروف الهجائية
تحذير
التحقق من صحة
جار التحقق من صحة المعلومات ...
نجاح
تحذير
فشل
يجب أن يكون مزيج من الأرقام & أمب؛ الحروف الهجائية
شريط التقدم
شريط التقدم
شريط تقدم قياسي.
شريط التقدم الدائري
شريط تقدم دائري.
ميني حجم شريط التقدم
مناسبة لمنطقة ضيقة.
شريط تقدم دائري أصغر.
ديناميكية شريط التقدم دائرية
شريط التقدم الديناميكي هو أفضل.
تنسيق نص مخصص
يمكنك تنسيق النص المخصص عن طريق وضع التنسيق.
لوحة القيادة
نمط لوحة القيادة للتقدم.
وصفت
نوع الزر
رمز الزر
ابتدائي
افتراضي
متقطع
خطر
بحث
بحث
بحث
بحث
حجم الزر
تم تعطيل الزر
زر التحميل
زر متعددة
زر المجموعة
علامات التبويب
بحث
تم تعطيل علامات التبويب
أيقونة علامات التبويب
ميني علامات التبويب
علامات تبويب إضافية للعمل
موضع
موضع علامات التبويب: يسار أو يمين أو أعلى أو أسفل
نوع بطاقة علامات التبويب
إضافة علامات التبويب وإغلاقها
عمودي نوع علامات التبويب
علامات التبويب الأساسية
مربع
مربع الاختيار الأساسي
الاستخدام الأساسي لخانة الاختيار.
مربع الاختيار
إنشاء مجموعة من مربعات الاختيار من صفيف. استخدم تعطيل لتعطيل مربع اختيار.
مربع الاختيار
إنشاء مجموعة من مربعات الاختيار من صفيف. استخدم تعطيل لتعطيل مربع اختيار.
راديو
الراديو الأساسي
أبسط استخدام. استخدام تعطيل لتعطيل الراديو.
عمودي راديوغروب
عمودي راديوغروب، مع المزيد من أجهزة الراديو.
RadioGroup
مجموعة من مكونات الراديو.
RadioGroup
مجموعة من مكونات الراديو.
تحويل
نقل مع مربع البحث.
بحث
الإكمال التلقائي
حسب الطلب
هل يمكن أن تمر AutoComplete.Option كأطفال الإكمال التلقائي، بدلا من استخدام داتاسورس
تخصيص مكون الإدخال
تخصيص مكون الإدخال
شارة
مثال أساسي
أبسط الاستخدام. سيتم إخفاء شارة عند العد هو 0، ولكن يمكننا استخدام شوزيرو لإظهار ذلك.
تجاوز عدد
يتم عرض أوفيرفلوكونت عندما يكون عدد أكبر من أوفيرفلوكونت. القيمة الافتراضية ل أوفيرفلوكونت هي 99.
الحالة
شارة مستقلة مع الوضع.
نجاح
خطأ
افتراضي
معالجة
تحذير
شارة حمراء
وهذا ببساطة عرض شارة حمراء، دون عدد معين.
ربط شيء
بطاقات
البطاقة الأساسية
بطاقة أساسية تحتوي على عنوان ومحتوى ومحتوى زاوية إضافي.
أكثر من
عنوان البطاقة
محتوى البطاقة
لوريم إيبسوم دولور سيت أميت، كونسكتيتور أديبيسيسينغ إليت، سيد دو إيسمود تيمبور إنسيدونت أوت لابور إت دولور ماغنا أليكوا. أوت إنيم أد مينيم فينيام، كويس نوسترود إكسيرسيساتيون أولامكو لابوريس نيسي أوت أليكيب إكس إي كومودو ثوسكات.
ليس لها حدود
بطاقة بلا حدود على خلفية رمادية.
بطاقة الشبكة
بطاقات عادة ما تتعاون مع تخطيط الشبكة في صفحة نظرة عامة.
تحميل البطاقة
لعرض مؤشر التحميل أثناء جلب محتويات البطاقة.
أيا كان المحتوى
محتوى مخصص
لعرض مؤشر التحميل أثناء جلب محتويات البطاقة.
فاز شارع أوروبا
www.instagram.com
إحتفال صاخب مخمور
دائري عمودي
ترقيم الصفحات الرأسي. استخدم {فرتيكال = 'ترو'}
دائري الأساسي
الاستخدام الأساسي
تتلاشى في الانتقال
الشرائح استخدام تتلاشى للانتقال. {تأثير = 'تتلاشى'}
مرر تلقائيا
توقيت التمرير إلى بطاقة المقبل / الصورة. تشغيل تلقائي
انهدام
يمكن توسيع أكثر من لوحة واحدة في وقت واحد، يتم تهيئة اللوحة الأولى لتكون نشطة في هذه الحالة. وس {ديفولتاكتيفيكي = {['كينوم']}}
الكلب هو نوع من الحيوانات المستأنسة. تشتهر الولاء والإخلاص، ويمكن العثور عليها كضيف ترحيب في العديد من الأسر في جميع أنحاء العالم.
هذا هو رأس اللوحة 1
هذا هو رأس اللوحة 2
هذا هو رأس اللوحة 3
هذا هو لوحة لوحة العش
مثال متداخل
يتم تداخل كولابس داخل كولابس.
مثال بلا حدود
نمط بلا حدود من كولابس. وس {بوردرد = {فالس}}
أكورديون
وضع الأكورديون، لوحة واحدة فقط يمكن توسيعها في وقت واحد. سيتم توسيع اللوحة الأولى بشكل افتراضي. استخدام الأكورديون
البوب
مثال أساسي
أبسط مثال. حجم طبقة عائمة يعتمد على محتويات المنطقة.
مرر لي
عنوان
ثلاث طرق لتحريك
الماوس فوق، والتركيز والتحرك في.
ركزني
انقر فوق لي
تحديد مستوى
تتوفر 12 خيارا للمواضع.
أعلى
أعلى اليسار
اعلى اليمين
أعلى اليسار
اليسار
أسفل اليسار
أعلى اليمين
حق
الأسفل
أسفل اليسار
أسفل يمين
التحكم في إغلاق مربع الحوار
استخدم دعامة مرئية للتحكم في عرض البطاقة.
TR
TL
LT
رطل
RT
RB
BL
BR
قريب
تلميح
تلميح المحتوى
مثال أساسي
أبسط الاستخدام.
تحديد مستوى
يحتوي تولتيب على 12 اختيار للمواضع.
TL
TR
LT
رطل
RT
RB
BL
BR
الأسفل
حق
اليسار
أعلى
سيتم عرض تلميح عندما يدخل الماوس.
تلميح المحتوى
الكلمات
مثال أساسي
استخدام العلامة الأساسية، ويمكن أن يكون قابلا للإغلاق عن طريق مجموعة الممتلكات القابلة للإغلاق. كلوسابل تدعم العلامة إغلوس بعد أحداث كلوس.
علامة 1
علامة 2
حلقة الوصل
منع افتراضي
علامة ملونة
العلامات الساخنة
حدد الموضوعات المفضلة لديك.
السواخن:
إضافة & إزالة ديناميكيا
إنشاء مجموعة من العلامات من قبل مجموعة، يمكنك إضافة وإزالة حيوي. على أساس الحدث بعد كلوس، والتي سيتم تشغيلها في حين نهاية الرسوم المتحركة وثيق.
+ علامة جديدة
الجدول الزمني
مثال أساسي
الجدول الزمني الأساسي
العقدة الأخيرة
عندما يكون الجدول الزمني غير مكتمل ومستمر، ضع عقدة شبح في الماضي. سيت {بندينغ = {ترو}} أور {بندينغ = {a رياكت إليمنت}}
شاهد المزيد
العادة
عين عقدة كرمز أو عنصر مخصص آخر.
مثال على الألوان
تعيين لون الدوائر. الأخضر يعني الانتهاء أو حالة النجاح، أحمر يعني التحذير أو الخطأ، والأزرق يعني استمرار أو حالة افتراضية أخرى.
إنشاء موقع خدمات 2015-09-01
حل مشاكل الشبكة الأولية 2015-09-01
مشاكل الشبكة التي يتم حلها 2015-09-01
الاختبار التقني 2015-09-01
اسقاط
مرر لأسفل
مرر لي
تحوم التنسيب المنسدلة
مرر مؤشر الماوس لأسفل مع تعطيل الرابط
النقر المنسدلة
زر مع القائمة المنسدلة
ترقيم الصفحات
الأساسية
أكثر من
المغير
سترة او قفاز او لاعب قفز
حجم صغير
الوضع البسيط
خاضع للسيطرة
الرقم الإجمالي
تقييم
مثال أساسي
أبسط الاستخدام.
نصف نجوم
دعم حدد نصف النجم.
عرض كتابة النصوص
إضافة مكونات الكتابة في معدل.
يقرأ فقط
للقراءة فقط، لا يمكن استخدام الماوس للتفاعل.
شخصية أخرى
استبدال النجمة الافتراضية إلى حرف آخر مثل الأبجدية، أرقام، إيكونفونت أو حتى كلمة الصينية.
شجرة
مثال أساسي
الاستخدام الأساسي، نخبرك بكيفية استخدام قابل للتحديد، قابل للتعديل، تعطيل، ديفولتكسانديكيس، وما إلى ذلك.
مثال أساسي يتم التحكم فيه
مثال أساسي يتم التحكم فيه
مثال قابل للسحب
اسحب ترينود لإدراج بعد ترينود أخرى أو إدراج في ترينود الأصل الآخر.
تحميل البيانات بشكل غير متزامن
لتحميل البيانات بشكل غير متزامن عند النقر لتوسيع ترينود.
مثال قابل للبحث
شجرة قابلة للبحث
شجرة مع خط
نيتسكيب 2.0 السفن، وتقديم جافا سكريبت
جيسي جيمس غاريت يطلق مواصفات أجاكس
جكري 1.0 صدر
تلتزم suberscore.js الأولى
Backbone.js يصبح شيئا
الزاوي 1.0 صدر
رد فعل مفتوح المصدر؛ المطورين نفرح
قائمة
شبكة
تصاعدي
تنازلي
خلط
استدارة
اضافة عنصر
إزالة بند
اتصالات بحث
إضافة جهة اتصال جديدة
اختر لونا لملاحظتك
إضافة ملاحظة جديدة
404
يبدو أنك قد فقدت
الصفحة التي تبحث عنها غير موجودة أو تم نقلها.
عودة المنزل
500
خطأ في الخادم الداخلي
هناك خطأ ما. يرجى إعادة المحاولة.
عودة المنزل
متماثل
هل نسيت كلمة المرور؟
أدخل بريدك الإلكتروني ونرسل إليك رابط إعادة تعيين.
ارسل طلب
متماثل
إعادة تعيين كلمة المرور
أدخل كلمة مرور جديدة وقم بتأكيدها.
حفظ
متماثل
تذكرنى
تسجيل الدخول
اسم المستخدم: تجريبي، كلمة السر: ديموديمو، أو فقط اضغط على أي زر.
قم بتسجيل الدخول باستخدام الفيسبوك
سجل الدخول باستخدام غوغل بلوس
سجل الدخول باستخدام أوث0
هل نسيت كلمة المرور
إنشاء حساب إسومورفويك
متماثل
أوافق على الشروط والأحكام
سجل
اشترك عبر حساب فايسبوك
اشترك مع غوغل بلوس
اشترك باستخدام أوث0
هل لديك حساب بالفعل؟ تسجيل الدخول.
الإيرادات
لوريم إيبسوم دولور سيت أميت، كونسكتيتور أديبيسيسينغ إليت، سيد دو إيسمود تيمبور
تسويق
Addvertisement
الاستشارات
تطوير
210
بريد إلكتروني غير مقروء
1749
تحميل الصور
3024
إجمالي الرسالة
54
أوامر المشاركة
الإيرادات
$ 15000
لوريم إيبسوم دولور سيت أميت، كونسكتيتور أديبيسيسينغ إليت، سيد دو إيسمود تيمبور
الإيرادات
$ 15000
لوريم إيبسوم دولور سيت أميت، كونسكتيتور أديبيسيسينغ إليت، سيد دو إيسمود تيمبور
الإيرادات
$ 15000
لوريم إيبسوم دولور سيت أميت، كونسكتيتور أديبيسيسينغ إليت، سيد دو إيسمود تيمبور
الإيرادات
$ 15000
لوريم إيبسوم دولور سيت أميت، كونسكتيتور أديبيسيسينغ إليت، سيد دو إيسمود تيمبور
110
رسائل جديدة
100٪
الصوت
137
موهلات
تحميل
50٪ كاملة
الدعم
80٪ العملاء راض
تحميل
65٪ كاملة
جون دو
مطور يوس
لوريم إيبسوم دولور سيت أميت، كونسكتيتور أديبيسيسينغ إليت، سيد دو إيسمود تيمبور أمت رولر كونسكتيتور أديبيسيسينغ إليت
الاسم الاول
الكنية
اسم الشركة
عنوان البريد الإلكتروني
رقم المحمول
بلد
مدينة
عنوان
شقة، جناح، وحدة الخ (اختياري)
انشئ حساب؟
صورة
الاسم الاول
الكنية
مدينة
شارع
البريد الإلكتروني
تاريخ الميلاد
إرسال
إلغاء
مؤلف موسيقى
يرجى تحديد بريد للقراءة
شراء الآن
إعدادات
`;
export default txt;
